...
exports.getAll = async (req, res) => {
  try {
    res.json(await Seats.find().populate('concert'));
  }
  catch (err) {
...
exports.getById = async (req, res) => {
  try {
    const s = await Seats.findById(req.params.id).populate('concert');
...
exports.post = async (req, res) => {
  ...
  try {
    ...
    const isTaken = await Seats.exists({concert: concert, seat: seat});
    if (isTaken) {
      res.status(409).json({ message: 'The seat is already taken...'});
    } else {
        const newSeat = new Seats({ seat, client, email, concert});
        await newSeat.save();
        res.json({ message: 'OK' });   
      }
  }
  ...
  try {
      ...
      if (isTaken) {
        res.status(409).json({ message: 'The seat is already taken...'});
          ...
          const s = await (Seats.findById(req.params.id));
          if (s) {
