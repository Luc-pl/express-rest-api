...
class SeatChooser extends React.Component {
  ...
  componentDidMount() {
    ...
    this.setState({ intervalidId: intervalId});
  }
  ...
  render() {
    ...
    const freeSeatsCount = seatsCount - seats.filter(item => item.day === chosenDay).length;
    ...
    return (
      <div>
        ...
        <h4>{`Free seats: ${freeSeatsCount}/${seatsCount}`}</h4>
      </div>
    )
  };
}
